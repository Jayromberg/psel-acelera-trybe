...
export default class JuridicalAccount extends Account {
  ...
  private validateCNPJ(CNPJ: string) {
    ...
    if (digit0 !== digits[0]) throw new BadRequestError("CNPJ inválido");

    // Valida 2o. dígito verificador
    const digit1 = this.validCalc(13, numbers);
    if (digit1 !== digits[1]) throw new BadRequestError("CNPJ inválido");

    // Formatação do CNPJ: 99.999.999/9999-99
    const formattedCNPJ = this.formatCNPJ(numbers);
    if (this.checkBlacklist(formattedCNPJ))
